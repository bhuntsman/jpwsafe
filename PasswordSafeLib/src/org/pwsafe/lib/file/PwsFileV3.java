		...
		try
		{
			...
			for (Iterator iter = getRecords(); iter.hasNext();) {
				...
				if (!rec.isHeaderRecord())
					rec.saveRecord(bos, this);
			}
			
			bos.write(PwsRecordV3.EOF_BYTES_RAW);
			bos.write(hasher.doFinal());
	
			bos.close();
	
			success = storage.save(bos.getBytes());
...
//				return;
//			}
			if (!success) throw new IOException("Unable to save data");
		}
		catch ( IOException e )
		{
			try
	 ...
	 */
	protected void readExtraHeader( InputStream is, PwsFile file )
	throws EndOfFileException, IOException, UnsupportedFileVersionException
	{
		//headerRecord = (PwsRecordV3) readRecord();
		headerRecord = new PwsRecordV3(is, this, true);
	}
	 ...
	 */
	protected void writeExtraHeader( OutputStream os, PwsFile file )
	throws IOException
	{
		headerRecord.saveRecord(os, this);
	}
	 ...
	 */
	protected void readDecryptedBytes( InputStream is, byte [] buff )
	 ...
	 * @throws IOException
	 */
	protected void writeEncryptedBytes( OutputStream os, byte [] buff )
	throws IOException
	{
		if ( (buff.length == 0) || ((buff.length % getBlockSize()) != 0) )
		{
