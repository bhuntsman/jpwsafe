/*
 * Copyright (c) 2008 David Muller <roxon@users.sourceforge.net>.
 * All rights reserved. Use of the code is allowed under the
 * Artistic License 2.0 terms, as specified in the LICENSE file
 * distributed with this code, or available from
 * http://www.opensource.org/licenses/artistic-license-2.0.php
 */
package org.pwsafe.lib.file;

import java.io.BufferedReader;
import java.io.FileInputStream;
import java.io.InputStream;
import java.io.InputStreamReader;

import junit.framework.TestCase;

import org.pwsafe.lib.exception.PasswordSafeException;
import org.pwsafe.lib.file.PwsS3Storage.Credentials;

public class PwsFileFactoryTest extends TestCase {
	protected void fillFileV2(PwsFileV2 file) throws PasswordSafeException {
		PwsRecordV2		rec;
		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 1") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 1") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Online Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 2") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 2") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Online Bank 2") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 3") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 3") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Lone entry") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 4") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 4") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Telephone Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.telephone") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 5") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 5") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Some Online Store") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "websites") );
		
		file.add( rec );

		assertEquals(5, file.getRecordCount());
	}
	protected void fillFileV3(PwsFileV3 file) throws PasswordSafeException {
		PwsRecordV3		rec;
		rec		= (PwsRecordV3) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV3.USERNAME, "User 1") );
		rec.setField( new PwsStringField( PwsRecordV3.PASSWORD, "Pass 1") );
		rec.setField( new PwsStringField( PwsRecordV3.TITLE, "Online Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV3.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV3) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV3.USERNAME, "User 2") );
		rec.setField( new PwsStringField( PwsRecordV3.PASSWORD, "Pass 2") );
		rec.setField( new PwsStringField( PwsRecordV3.TITLE, "Online Bank 2") );
		rec.setField( new PwsStringField( PwsRecordV3.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV3) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV3.USERNAME, "User 3") );
		rec.setField( new PwsStringField( PwsRecordV3.PASSWORD, "Pass 3") );
		rec.setField( new PwsStringField( PwsRecordV3.TITLE, "Lone entry") );
		rec.setField( new PwsStringField( PwsRecordV3.GROUP, "bank") );
		
		file.add( rec );

		rec		= (PwsRecordV3) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV3.USERNAME, "User 4") );
		rec.setField( new PwsStringField( PwsRecordV3.PASSWORD, "Pass 4") );
		rec.setField( new PwsStringField( PwsRecordV3.TITLE, "Telephone Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV3.GROUP, "bank.telephone") );
		
		file.add( rec );

		rec		= (PwsRecordV3) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV3.USERNAME, "User 5") );
		rec.setField( new PwsStringField( PwsRecordV3.PASSWORD, "Pass 5") );
		rec.setField( new PwsStringField( PwsRecordV3.TITLE, "Some Online Store") );
		rec.setField( new PwsStringField( PwsRecordV3.GROUP, "websites") );
		
		file.add( rec );

		assertEquals(5, file.getRecordCount());
	}
	public void testFile()
	throws PasswordSafeException
	{
		PwsFileV2	file;
		PwsRecordV2	rec;

		file	= new PwsFileV2();
		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 1") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 1") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Online Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 2") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 2") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Online Bank 2") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.online") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 3") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 3") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Lone entry") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 4") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 4") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Telephone Bank 1") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "bank.telephone") );
		
		file.add( rec );

		rec		= (PwsRecordV2) file.newRecord();

		rec.setField( new PwsStringField( PwsRecordV2.USERNAME, "User 5") );
		rec.setField( new PwsStringField( PwsRecordV2.PASSWORD, "Pass 5") );
		rec.setField( new PwsStringField( PwsRecordV2.TITLE, "Some Online Store") );
		rec.setField( new PwsStringField( PwsRecordV2.GROUP, "websites") );
		
		file.add( rec );

		assertEquals(5, file.getRecordCount());
		
		//TODO proper tests here
	}
	
	/* So far, this test just makes sure no exceptions are thrown */
	public void testFileStorage() throws Exception {
		PwsFileStorage pfs = new PwsFileStorage("password_file_2.dat");
		byte[] data = pfs.load();
	}

	public void testS3Writing() throws Exception {
		PwsFileV3 pwsFileV3 = new PwsFileV3();
		pwsFileV3.setPassphrase("Amazon");
		pwsFileV3.setStorage(PwsS3Storage.fromFile(pwsFileV3, "pwsafe.ps3"));
		pwsFileV3.save();
	}
	
	/* So far, this test just makes sure no exceptions are thrown */
	public void testS3Reading() throws Exception {
		PwsS3Storage pss = PwsS3Storage.fromFile(null, "pwsafe.ps3");
		byte[] data = pss.load();
	}

	public void testLoadFile () throws Exception {
		PwsFile theFile = PwsFileFactory.loadFile("password_file_2.dat", "THEFISH");
		
		assertNotNull(theFile);
		assertTrue(theFile instanceof PwsFileV2);
		
		assertEquals (1, theFile.getRecordCount());
	}
}
