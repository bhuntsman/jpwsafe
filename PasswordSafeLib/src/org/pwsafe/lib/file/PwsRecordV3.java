...
public class PwsRecordV3 extends PwsRecord
{
	 ...
	 */
	PwsRecordV3( InputStream is, PwsFile file )
	throws EndOfFileException, IOException
	{
		super( is, file, VALID_TYPES );
	}
	 ...
	 * @throws IOException        If a read error occurs.
	 */
	PwsRecordV3( InputStream is, PwsFile file, boolean ignoreFieldTypes)
	throws EndOfFileException, IOException	{
		super(is, file, VALID_TYPES, ignoreFieldTypes);
	}
...
//	throws CloneNotSupportedException
	...
	protected class ItemV3 extends Item {
		public ItemV3( InputStream is, PwsFileV3 file )
		throws EndOfFileException, IOException
		{
			super();
			try {
			...
			if (Length <= 11) {
				...
				for (int i = 0; i < blocksToRead; i++) {
					...
					file.readDecryptedBytes( is, nextBlock );
					if (i == blocksToRead - 1) {
						// last block, do magic
	 ...
	 * @throws IOException
	 */
	protected void loadRecord( InputStream is, PwsFile file )
	throws EndOfFileException, IOException
	{
		Item		item;
		...
		for ( ;; )
		{
			item = new ItemV3( is, (PwsFileV3)file );
...
//					
	 ...
	 */
	protected void saveRecord(OutputStream os, PwsFile file)
	throws IOException
	{
		...
		for ( Iterator iter = getFields(); iter.hasNext(); )
		{
			...
			writeField( os, file, value );
			
			PwsFileV3 fileV3 = (PwsFileV3) file;
			fileV3.hasher.digest(value.getBytes());
		}
		writeField( os, file, new PwsStringField( END_OF_RECORD, "" ) );
		LOG.debug2( "----- END OF RECORD -----" );
	}
	...
	throws IOException
	{
		...
		for (int i = 0; i < (dataToWrite.length / 16); i++) {
			...
			file.writeEncryptedBytes(os, nextBlock);
		}
