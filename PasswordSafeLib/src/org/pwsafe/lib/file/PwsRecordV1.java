...
public class PwsRecordV1 extends PwsRecord implements Comparable
{
	 ...
	 */
	PwsRecordV1( InputStream is, PwsFile file )
...
//	throws CloneNotSupportedException
	...
	throws EndOfFileException, IOException
	{
		...
		setField( new PwsStringField(PASSWORD, new Item(is, file).getData()) );
		setField( new PwsStringField(NOTES, new Item(is, file).getData()) );
	}
	 ...
	 */
	protected void saveRecord(OutputStream os, PwsFile file) throws IOException
	{
		PwsField	title;
		...
		writeField( os, file, title, DEFAULT_TYPE );
		writeField( os, file, getField(PASSWORD), DEFAULT_TYPE );
		writeField( os, file, getField(NOTES), DEFAULT_TYPE );
	}
