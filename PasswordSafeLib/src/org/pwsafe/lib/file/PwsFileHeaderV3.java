...
//		}
	 ...
	 */
	public PwsFileHeaderV3( InputStream is, PwsFile file )
	throws IOException, EndOfFileException
	{
		file.readBytes( is, tag );
		file.readBytes( is, salt );
		file.readBytes( is, iter );
		file.readBytes( is, password );
		file.readBytes( is, b1 );
		file.readBytes( is, b2 );
		file.readBytes( is, b3 );
		file.readBytes( is, b4 );
		file.readBytes( is, IV );
	}
	 ...
	 * @throws IOException
	 */
	public void save( OutputStream os, PwsFile file )
	throws IOException
	{
		...
		file.writeBytes( os, tag );
		file.writeBytes( os, salt );
		file.writeBytes( os, iter );
		file.writeBytes( os, password );
		file.writeBytes( os, b1 );
		file.writeBytes( os, b2 );
		file.writeBytes( os, b3 );
		file.writeBytes( os, b4 );
		file.writeBytes( os, IV );
