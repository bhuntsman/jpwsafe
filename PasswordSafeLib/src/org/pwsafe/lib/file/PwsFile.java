...
//		
//	}
	/** Returns the file storage object */
	public PwsStorage getStorage() { return storage; }
	 ...
	 */
	protected byte [] readBlock(InputStream is)
	throws EndOfFileException, IOException
	{
		byte []	block;

		block = new byte[ getBlockSize() ];
		readDecryptedBytes( is, block );
	 ...
	 * @throws IOException        If an error occurs whilst reading the file.
	 */
	protected void readBytes( InputStream is, byte [] bytes )
	...
	throws IOException, NoSuchAlgorithmException
	{
		...
		try
		{
			...
			bos.close();
			if (storage.save(bos.toByteArray())) {
				for ( Iterator iter = getRecords(); iter.hasNext(); )
				{
					rec = (PwsRecord) iter.next();
		...
		if (!success) throw new IOException("failed to save data");
	}
	 ...
	 * @throws IOException
	 */
	void writeBytes( OutputStream os, byte [] buffer )
	throws IOException
	{
		os.write( buffer );
